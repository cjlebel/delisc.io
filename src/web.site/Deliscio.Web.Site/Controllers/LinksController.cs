...
public class LinksController : BaseController<LinksController>
{
    ...
    private const string SKIP_GREATER_THAN_ZERO = "Offset must be greater or equal to 0.";
    ...
    public async Task<IActionResult> Index(
        [FromQuery] int? p = 1, [FromQuery] int? s = 0, 
        [FromQuery] string? t = "", 
        CancellationToken token = default)
    {
        var pageNo = p.GetValueOrDefault(1);
        var offset = s.GetValueOrDefault(0);
        var tags = t?.Trim();
        ...
        try
        {
            var model = await _linksManager.GetLinksPageViewModelAsync(pageNo, offset, tags, token);
    ...
    [Route("links/{id}")]
    public async Task<IActionResult> Details([FromRoute] string id, CancellationToken token = default)
    {
