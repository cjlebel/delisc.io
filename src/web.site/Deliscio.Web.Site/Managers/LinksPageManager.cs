...
public interface ILinksPageManager
{
    ...
    /// <param name="offset">The number of items to offset when applicable (default 0)</param>
    /// <param name="tags">A string of tags to filter by</param>
    /// <param name="token"></param>
    /// <returns></returns>
    Task<LinksPageViewModel?> GetLinksPageViewModelAsync(int? pageNo = 1, int? offset = 0, string? tags = default, CancellationToken token = default);

    Task<LinksDetailsPageViewModel?> GetLinksDetailsPageViewModelAsync(string linkId, CancellationToken token = default);
}
...
public class LinksPageManager : BasePageManager, ILinksPageManager
{
    ...
    public async Task<LinksPageViewModel?> GetLinksPageViewModelAsync(int? pageNo = 1, int? offset = 0, string? tags = default, CancellationToken token = default)
    {
        var page = pageNo.GetValueOrDefault(1);
        var skipCount = Math.Max(offset.GetValueOrDefault(0), 0);
    ...
    public async Task<LinksDetailsPageViewModel?> GetLinksDetailsPageViewModelAsync(string linkId, CancellationToken token = default)
    {
    ...
    /// <summary>
    /// Builds the query string for the links page with the page number, offset count, and tags
    /// </summary>
